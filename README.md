...
| 连接时提示**指定的网络名不再可用（`0x00000040`）** | 到对方的旧 SMB 会话已失效，或名称还指向旧 IP（DHCP 变更、缓存未刷新） | 客户端：清理旧 SMB 连接 + 刷新 DNS / NetBIOS 缓存；并用 `\\IP\共享名` 验证 |
| 提示同一台电脑存在**多重连接冲突**（`1219`） | 到对方存在多条凭据不同的连接 | 客户端：先删掉到它的旧连接再连 |
