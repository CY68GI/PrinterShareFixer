...
界面上只有两个主要按钮——「修复 Windows 10 共享」和「修复 Windows 11 共享」，
点击后自动完成服务启用、网络发现、防火墙放行、打印 RPC 兼容性等一整套修复动作，
并逐项显示每一步的执行结果。

## 界面

- **顶部**：系统状态检测。显示操作系统版本、管理员权限、关键服务状态、网络位置、
  防火墙规则、SMB 来宾登录、打印 RPC 隐私认证、NetBIOS、已共享的打印机等，
  并自动在推荐按钮上标记「当前系统推荐」。
- **中间**：两个修复按钮（Windows 10 / Windows 11）。
- **高级选项**：注册表备份、来宾访问、SMB 签名、RPC 动态端口、防火墙远程范围、
  SMB1、受保护的打印模式、修复后重启服务。
...
| 服务配置 | `LanmanServer`、`LanmanWorkstation`、`Spooler` 设为自动并启动；`FDResPub`、`fdPHost`、`SSDPSRV`、`upnphost`、`Dnscache`、`NlaSvc`、`netprofm`、`Netman`、`mpssvc`、`RpcSs`、`RpcEptMapper`、`RpcLocator` 按需启动 |
...
│  ├─ PrinterShareFixer-1.0.0-win-x64.zip  一个压缩包包含全部内容，拷到其他电脑解压即用
│  └─ 使用说明.txt                       给最终用户的使用说明
...
仓库已经初始化（分支 `main`），并打好了 `v1.0.0` 标签：
...
```powershell
dotnet run --project .\src\PrinterShareFixer.Cli -- detect             # 只读检测
dotnet run --project .\src\PrinterShareFixer.Cli -- detect --verbose    # 检测并打印详细日志
dotnet run --project .\src\PrinterShareFixer.Cli -- plan win11         # 查看修复计划与等价命令
dotnet run --project .\src\PrinterShareFixer.Cli -- run win11          # 预演，不做修改
dotnet run --project .\src\PrinterShareFixer.Cli -- run win11 --yes    # 真正执行（需管理员权限）
```

打包后也可以直接用 `release\psfix-cli-win-x64\psfix.exe`。

高级选项用 `--opt key=value` 传入，例如 `--opt smb1=true --opt anyremote=true --opt wpp=true`。
